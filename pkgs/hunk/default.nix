# Vendored from the local nixpkgs checkout (pkgs/by-name/hu/hunk/package.nix,
# branch kaynetik/tmp-hunk), bumped to 0.17.0. Drop this package and the
# `hunk` overlay entry in flake.nix once a `nix flake update` brings hunk
# into the locked nixpkgs-darwin.
{
  lib,
  stdenv,
  bun,
